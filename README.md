...
一个无数据库、无 Google Sheets 的 Streamlit 工资结算工具。继续使用原有的横向 Excel 排班方式，上传后会转换成按日期横向排列的完整 24 小时排班，再直接生成适合发工资的汇总与明细。

## 使用流程

1. 在 Excel、WPS 或 LibreOffice 中照常完成横向排班，并保存为 `.xlsx` 或 `.xlsm`。
2. 上传排班表，系统会自动建议与排班日期最匹配的活动；也可以手动选择上一期或其他活动。
3. 切换班组后可在同一张表里查看全部日期并直接修改人员。时间列固定，岗位列紧凑显示，夜班行使用浅灰色。
4. 检查岗位工价与白班、夜班开始时间。
...
- 各日期按列组横向排列；表格第一行的日期选择器仅在浏览器内快速定位，不会重新加载页面。
